library(stringr)
oxcal_out <- readLines("~/OxCal/kde_reg_500.js")
...
kde_sdate <- lapply(str_extract_all(oxcal_out[kdestartdate],"[-][:alnum:]*|[:alnum:]*"),function(x){as.numeric(x[[10]])})
...
kdepost <- data.frame(Density=kdepost,
                     Date=seq(unlist(kde_sdate),length.out=length(kdepost)) )
...
cal_curve_df <- data.frame(RCBP=curve_fun_bp,
                           ...
                           BCAD=seq(curve_fun_bcad_start,1951))
...
t_seq <- lapply(1:nkdes,function(x,t_seq_length,kde_sdates){c(seq(kde_sdates[[x]],kde_sdates[[x]] + t_seq_length[[x]] - 1))},t_seq_length=t_seq_length,kde_sdates=kde_sdates)

kde_ensembles <- lapply(1:nkdes,function(x,p,ts){cbind(ts[[x]],p[[x]])},p=kde_ensembles,ts=t_seq)
